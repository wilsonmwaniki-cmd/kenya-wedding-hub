-- Recreate the public budget estimator RPC and explicitly reload the
-- PostgREST schema cache so preview and production deployments can see the
-- current 5-argument function signature immediately.
