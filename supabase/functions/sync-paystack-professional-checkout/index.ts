...
serve(async (req) => {
  ...
  try {
    ...
    if (audience !== 'planner' && audience !== 'vendor') throw Object.assign(new Error('Invalid professional audience.'), { status: 400 });
    ...
    if (transaction.audience !== audience) throw Object.assign(new Error('This Paystack transaction does not match the requested professional audience.'), { status: 400 });
    ...
    const mapping = config.professionalCheckoutMap[transaction.lookup_key];
    if (!mapping) throw Object.assign(new Error('This Paystack transaction is not a supported Professional plan.'), { status: 400 });
    ...
    await logFunctionEvent({
      functionName: 'sync-paystack-professional-checkout',
      severity: 'info',
      status: 'success',
      eventType: 'checkout_sync_succeeded',
      message: `Activated professional Paystack purchase ${transaction.lookup_key}.`,
      userId: user.id,
      audience: transaction.audience,
      requestId,
      details: { reference, activatedFeatures: result.activatedFeatures, seatLimit: result.seatLimit },
    });
    ...
    const status = isAuthSessionError(error)
      ...
      : typeof (error as { status?: unknown })?.status === 'number'
        ...
        : 500;
    const message = error instanceof Error ? error.message : 'Unknown Paystack professional checkout sync error';
    await logFunctionEvent({
      functionName: 'sync-paystack-professional-checkout',
